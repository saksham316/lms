...
import { addPdfQuiz, getPdfQuizzes } from "../../features/actions/StudyMaterial/studyMaterialActions";
...
const StudyMaterial = () => {
  ...
  const { isStudyMaterialAdded, isLoading } = useSelector(
    (state) => state?.studyMaterial
  );

  // -------------------------------------------------------------------------------------------------------------
  // ---------------------------------------------------Functions-------------------------------------------------
  // createStudyMaterial -- createStudyMaterial in order to call the create study material api
  const createStudyMaterial = (data) => {
    try {
      confirmAlert({
        title: "NOTE!",
        message: "Are you sure! You want to Create the Study Material",
        buttons: [
          {
            label: "Yes",
            onClick: () => {
              let formData = new FormData();
              const { pdfName, pdfDescription, pdfFile } = data;
      ...
      <Container>
        <Row>
          <Col className="">
            ...
            <form onSubmit={handleSubmit(createStudyMaterial)}>
              ...
              <div>
                <h1 className={styles.title}>Add PDF</h1>
              </div>
              <div className={`${styles.thumbnail}`}>
                ...
                {errors.pdfFile && (
                  <div className="text-danger pt-1">
                    {errors.pdfFile.message || "PDF File is required"}
                  </div>
                )}
              </div>
              <div>
